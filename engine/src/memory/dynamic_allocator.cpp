    ...
    if (state->MemoryBlock && size) {
        ...
        if (state->list.AllocateBlock(RequiredSize, BaseOffset)) {
            ...
            */
            ...
            // Начните выравнивание после достаточного пространства для хранения u32. Это позволяет хранить u32
            // непосредственно перед блоком пользователя, сохраняя при этом выравнивание на указанном блоке пользователя.
