...
public:
    ...
    /// @brief Пытается найти выровненный по памяти свободный блок памяти заданного размера и детализации.
    /// @param size Размер для выделения.
    /// @param alignment Количество байтов для выравнивания.
    /// @param OutOffset Указатель для хранения смещения на выровненную выделенную память.
    /// @param OutAlignmentOffset Указатель для хранения внутреннего смещения выравнивания в байтах.
    /// @return True, если блок памяти был найден и выделен; в противном случае false.
    bool AllocateBlockAligned(u64 size, u16 alignment, u64& OutOffset, u16& OutAlignmentOffset);
    ...
    /// @brief Пытается освободить блок выровненной памяти по указанному смещению и указанного размера. Может завершиться неудачей, если переданы неверные данные.
    /// @param size Размер для освобождения.
    /// @param offset Смещение для освобождения.
    /// @param AlignmentOffset Смещение выравнивания.
    /// @return True в случае успеха; в противном случае false. False следует рассматривать как ошибку.
    bool FreeBlockAligned(u64 size, u64 offset, u16 AlignmentOffset);
