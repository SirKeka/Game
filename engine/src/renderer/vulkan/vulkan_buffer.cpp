...
VulkanBuffer::VulkanBuffer(VulkanAPI *VkAPI, u64 size, VkBufferUsageFlagBits usage, u32 MemoryPropertyFlags, bool BindOnCreate, bool UseFreelist)
    ...
    FreeListBlock(UseFreelist ? MMemory::Allocate(FreeListMemoryRequirement, Memory::Renderer, true) : nullptr), 
    BufferFreeList(),
    ...
    bool UseFreelist)
{
    ...
    if (UseFreelist) {
        ...
        FreeListBlock = MMemory::Allocate(FreeListMemoryRequirement, Memory::Renderer, true);
        BufferFreeList.Create(size, FreeListBlock);
    }
...
bool VulkanBuffer::Resize(VulkanAPI *VkAPI, u64 NewSize, VkQueue queue, VkCommandPool pool)
{
    ...
    if (HasFreelist) {
        ...
        void* NewBlock = MMemory::Allocate(NewMemoryRequirement, Memory::Renderer, true);
