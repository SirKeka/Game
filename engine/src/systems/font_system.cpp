...
struct BitmapFontLookup {
    ...
    constexpr BitmapFontLookup() : id(INVALID::U16ID), ReferenceCount(INVALID::U16ID), font() {}
};
...
bool FontSystem::Initialize(FontSystemConfig& config)
{
    ...
    u16* BmpHashtableBlock = reinterpret_cast<u16*>(BmpArrayBlock + config.MaxBitmapFontCount);

    state = new(MemBlock) FontSystem(config, BmpArrayBlock, BmpHashtableBlock);
